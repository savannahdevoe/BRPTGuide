...
AnyOption::init(int maxopt, int maxcharopt )
{
	...
	if( alloc() == false ){
		cerr << endl << "OPTIONS ERROR : Failed allocating memory" ;
		cerr << endl ;
		cerr << "Exiting." << endl ;
		exit (0);
	}
...
{
	cerr << endl ;
	cerr << "OPTIONS ERROR : Failed allocating extra memory " << endl ;
	cerr << "While adding the option : \""<< opt << "\"" << endl;
	cerr << "Exiting." << endl ;
	cerr << endl ;
	exit(0);
}
...
char AnyOption::parsePOSIX( char* arg )
  ...
  /*  
	printVerbose( "Unknown POSIX command argument option : " );
	printVerbose( arg );
	printVerbose( );
	printAutoUsage();
  */
...
void AnyOption::addUsage(const char *fmt, ...)
  {
  char buf[1024];
  va_list args;
  va_start(args, fmt);
